...
$data = file_get_contents($file_path . '/cart.txt');
    ...
    if(ctype_digit($number) && ctype_digit($code)){
         $ordernumber = rand(10,10000);
         $userfile = fopen($file_path . "/".$fname.".txt", "a");
         fwrite($userfile, $email.",".$ordernumber."\n");
    }else{
        print 'no';
        header('Location: checkout.php?invalid=number');
...
--> 
    ...
    <body>
        ...
        <?php
          ...
          <div class="textbox">
        <?php
                print "Your order is confirmed! Your confirmation number is ".$ordernumber.".";
                setcookie('total', '', time()-3600);
                setcookie('quantity', '', time()-3600);
          ?>    
              <br>
              <br>
              <a href="orderhistory.php">View Order History Here</a>
            </div>
          <?php
          } else{ 
              print 'Session Timeout. Login in Again';
              ?>
